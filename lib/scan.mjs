...
export async function scanAudioFiles(path, opts) {
  const existingScanId = await findExistingScanId(path, opts.ext, opts.cacheScanTtl);
  if (existingScanId) {
...
async function findExistingScanId(path, ext, cacheScanTtl = 0) {
  ...
  const rows = await db
    .select({ id: Scans.id })
    .from(Scans)
    .where(and(eq(Scans.path, path), eq(Scans.ext, [...ext].sort().join()), gte(Scans.createdAt, createdAt)))
    .limit(1);
